  ...
  .post(
    ...
    async ({ body, headers, jwt, set }) => {
      ...
      if (String(payload.userId) !== String(body.userId)) {
        set.status = 403;
        return { success: false, message: "Forbidden: userId does not match token" };
      }
      ...
      try {
        ...
        if (!pc) {
          set.status = 404;
          return { success: false, message: `PC-${pcId} is not found` };
        }

        if (pc.currentUserId !== userId) {
          set.status = 403;
          return { success: false, message: `user ${user.username} isn't logged in at this PC` };
        }
        ...
        if (user.balance < finalPrice) {
          set.status = 403;
          return { success: false, message: `Insufficient balance (Need ${finalPrice} Coins)` };
        }
        ...
        await db.transaction(async (tx) => {
          ...
          const baseTime = currentEndTime > Date.now() ? currentEndTime : Date.now();
          const newEndTime = baseTime + durationMs;

          await tx
            .update(pcs)
            .set({ status: "online", currentUserId: user.id, sessionEndTime: newEndTime })
